...
/**
 * Source for a Windows taskbar icon (overlay icons, thumbnail toolbar buttons).
 */
sealed class TaskbarIconSource {
    /** Use a Windows Shell stock icon (available on all Windows Vista+ systems). */
    data class FromStock(val stockIcon: StockIcon) : TaskbarIconSource()

    /** Load an icon from an `.ico` file on disk. */
    data class FromFile(val path: String) : TaskbarIconSource()
     ...
     */
    data class FromResource(val dllPath: String, val index: Int) : TaskbarIconSource()
}
